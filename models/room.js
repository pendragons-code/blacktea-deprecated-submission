...
class Room {
	...
	removePlayer(playerID) {
		...
		if(this.players.length < 2 && this.gameState === "running") {
	...
	nextTurn(playerID) {
		...
		setTimeout(() => {
			if(this.currentTurn !== newPlayer) return;
			this.broadcast("timesup", { playerNumber: this.idToIndex(newPlayer) });
			this.sendToPlayer(newPlayer, "alert", "You took too long!");
			setTimeout(() => {
				return this.nextTurn(newPlayer);
			}, 1000);
	...
	startGame() {
		...
		setTimeout(() => {
			if(this.currentTurn !== this.creatorID) return;
			this.broadcast("timesup", { playerNumber: this.idToIndex(this.creatorID) }); // we can actually return player 1 instead of having to index, but there was 1 run we somehow had it register as player 2 and im not clear how
			console.log("timesup")
